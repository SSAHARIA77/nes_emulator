...
class PPU2C02{
    ...
    private:
     ...
     //First 4 palettes are generally used for background palettes and last 4 for foreground palettes.
    ...
    private:
        ...
        uint8_t address_latch = 0x00; // to know if we are using low byte or high byte
        uint8_t ppu_data_buffer = 0x00; // when we read from ppu it is delayed by one cyle so we need to buffer the byte
