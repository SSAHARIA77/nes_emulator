...
olc::Sprite &PPU2C02::GetPatternTable(uint8_t i, uint8_t palette) {
    for (uint16_t tileY{}; tileY < 16; tileY++) {
        for (uint16_t tileX{}; tileX < 16; tileX++) {
            ...
            // Note that width of one row of tiles is 16 tiles * 16 bytes = 256
            // bytes.
            ...
            for (uint16_t row{}; row < 8; row++) {
                ...
                for (uint16_t col{}; col < 8; col++) {
                    // Adding the value of LSB plane and MSB plane to get a
                    // value b/w 0 and 3 for the pixel
...
void PPU2C02::cpuWrite(uint16_t addr, uint8_t data) {
    switch (addr) {
        ...
        case 0x0006:  // PPU Address
            ...
            if (address_latch == 0) {
                tramAddr.reg =
                    uint16_t(((data & 0x3F) << 8) | (tramAddr.reg & 0x00FF));
                address_latch = 1;
            } else {
...
void PPU2C02::clock() {
    ...
    if (scanline >= -1 && scanline < 240) {
        ...
        if ((cycle >= 2 && cycle < 258) || (cycle >= 321 && cycle < 338)) {
            ...
            switch ((cycle - 1) % 8) {
                ...
                case 2:
                    bgNextTileAttribute =
                        ppuRead(0x23C0 | (vramAddr.nametable_y << 11) |
        ...
        if (cycle == 257 && scanline >= 0) {
            // Resetting
            std::memset(spriteScanline, 0xFF,
                        8 * sizeof(sObjectAttributeEntry));
            spriteCount = 0;
            ...
            while (nOAMEntry < 64 && spriteCount < 9) {
                ...
                if (diff >= 0 && diff < (control.sprite_size ? 16 : 8)) {
                    if (spriteCount < 8) {
                        ...
                        memcpy(&spriteScanline[spriteCount], &OAM[nOAMEntry],
        ...
        if (cycle == 340) {
            for (uint8_t i = 0; i < spriteCount; i++) {
                uint8_t spriteBitsPatternLo, spriteBitsPatternHi;
                uint16_t spriteAddrPatternLo, spriteAddrPatternHi;

                if (!control.sprite_size) {
                    // 8x8 Sprite Mode
                    if (!(spriteScanline[i].attribute & 0x80)) {
                        // Sprite is not vertically flipped, i.e. normal
                        // orientation

                        // Get the address
                        spriteAddrPatternLo = (control.pattern_sprite << 12) |
                    ...
                    // 8x16 Sprite Mode (basically dealing with 2 tiles)
                    if (!(spriteScanline[i].attribute & 0x80)) {
                        // Normal Orientation
                        if (scanline - spriteScanline[i].y < 8) {
                            // If we are in top tile
                            spriteAddrPatternLo =
                                ((spriteScanline[i].id & 0x01) << 12) |
                                ((spriteScanline[i].id & 0xFE) << 4) |
                                ((scanline - spriteScanline[i].y) & 0x07);
                        } else {
                            // If we are in bottom tile
                            spriteAddrPatternLo =
                                ((spriteScanline[i].id & 0x01) << 12) |
                ...
                spriteAddrPatternHi = spriteAddrPatternLo + 8;
                spriteBitsPatternLo = ppuRead(spriteAddrPatternLo);
                spriteBitsPatternHi = ppuRead(spriteAddrPatternHi);
                ...
                if(spriteScanline[i].attribute & 0x40){
                    ...
                    //Flip Horizontal
                    spriteBitsPatternLo = flipbyte(spriteBitsPatternLo);
                    spriteBitsPatternHi = flipbyte(spriteBitsPatternHi);
                }

                spriteShifterPatternLo[i] = spriteBitsPatternLo;
                spriteShifterPatternHi[i] = spriteBitsPatternHi;
            }
        }
    }

    // Rendering on hidden screen -> CPU can now interact with ppu
    // Basically when cathode ray tube moves back to the top, it is the time cpu
    // can make changes to the graphics through ppu to update the screen in the
    // next render cycle. This is done to avoid screen glitches and artifacts
    // that might be caused if the cpu writes in middle of render period.
    if (scanline == 240) {
    ...
    if(mask.render_sprites){
        ...
        for(uint8_t i = 0; i<spriteCount; i++){
            if(spriteScanline[i].x == 0){
                uint8_t fg_pixel_lo = (spriteShifterPatternLo[i] & 0x80) > 0;
                uint8_t fg_pixel_hi = (spriteShifterPatternHi[i] & 0x80) > 0;
                fgPixel = (fg_pixel_hi << 1) | fg_pixel_lo;

                fgPalette = (spriteScanline[i].attribute & 0x03) + 0x04;
                //Priority with other sprites is automatically handled as sprites go from 0 - 63.
                //NOT the priority wrto other sprites but with respect to background to allow sprites to go behind tiles in background
    ...
    // Basically we scrolling through the screen and when all rows and
    // columns are complete, the frame is complete
    if (cycle >= 341) {
        cycle = 0;
        scanline++;
        if (scanline >= 261) {
